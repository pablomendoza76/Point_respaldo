...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, BarraUbicacionComponent, BarraBusquedaComponent, FormularioDinamicoLoaderComponent, DeleteModalComponent],
...
export class TarifasPorGrupoComponent implements OnInit, OnDestroy {
  ...
  constructor(private tarifasService: TarifasService, private router: Router, private menuRoutesService: MenuRoutesService) {}
  ...
  filtrarGrupos(): void {
    ...
    this.grupos = valor ? this.gruposOriginales.filter((grupo) => grupo.nombre?.toLowerCase().includes(valor)) : [...this.gruposOriginales]
  ...
  guardarGrupo(): void {
    ...
    const peticion = this.esEdicion ? this.tarifasService.actualizarGrupo({ ...datos, codigo }) : this.tarifasService.agregarGrupo(datos)
